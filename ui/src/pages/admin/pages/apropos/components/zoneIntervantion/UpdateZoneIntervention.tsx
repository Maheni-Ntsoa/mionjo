...
interface UpdateZoneInterventionProps {
...
const UpdateZoneIntervention: React.FC<UpdateZoneInterventionProps> = ({
  zoneintervention,
  buttonActiver,
  onClose,
  refetch,
}) => {
  const [zonei, setzonei] = useState<any>({});

  useEffect(() => {
    if (zoneintervention) {
      setzonei(zoneintervention);
    }
  }, []);
  const [isSuccess, setIsSuccess] = useState(false); // État du succès du formulaire

  const initialValues = {
    titre: `${zoneintervention.titre ? zoneintervention.titre : ""}`,
    contenu: `${zoneintervention.contenu ? zoneintervention.contenu : ""}`,
  };
  ...
  return (
    <div className="px-4">
      <div className="flex justify-center">
        <p className="text-yellow">Modification Zone d'intervention</p>
      </div>
      <Formik
...
export default UpdateZoneIntervention;
